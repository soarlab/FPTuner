...
def expand_name(work_stack, count, exp):
  assert(len(exp) == 2)
...
default_walk_expand_func_dict["Return"] = expand_name
...
def constant_expand_one(work_stack, count, exp):
  assert(len(exp) == 3 or exp[0] == "Return")
  work_stack.append((False, 1,     exp[-1]))
...
def _walk(default_expand_dict, expand_dict,
  ...
  while len(work_stack) > 0:
    ...
    # Contract this op
      ...
      op_done, op_count, tup = work_stack.pop()
      assert(op_done == True)
      if tup in {"Variable", "Input", "Return"}:
        op = tup
      else:
