...
def ReadyToTune():
    ...
    assert(all([isinstance(TARGET_EXPRS[i], tft_expr.Expr)
                for i in range(0, len(TARGET_EXPRS))]))
    assert(EFORMS is not None)
    assert(len(EFORMS) > 0)
    assert(all([isinstance(ef, tft_error_form.ErrorForm) for ef in EFORMS]))


def GenerateErrorTermsFromExpr(context_expr, expr,
                               error_exprs = [], program_exprs = []):
    ...
    if (isinstance(expr, tft_expr.VariableExpr)
        ...
        for i in range(len(program_exprs)):
            pe = program_exprs[i]
        ...
        if isinstance(expr, tft_expr.UnaryExpr):
            ets_opd = GenerateErrorTermsFromExpr(expr,
                                                 expr.opd(),
                                                 error_exprs,
                                                 program_exprs)
        ...
        if (isinstance(expr, tft_expr.BinaryExpr)):
            ets_lhs = GenerateErrorTermsFromExpr(expr,
                                                 expr.lhs(),
                                                 error_exprs,
                                                 program_exprs)
            ets_rhs = GenerateErrorTermsFromExpr(expr,
                                                 expr.rhs(),
                                                 error_exprs,
                                                 program_exprs)
            return [my_et] + ets_lhs + ets_rhs

        logger.error("Broken control flow in GenerateErrorTermsFromExpr.")
        sys.exit(1)

    logger.error("Invalid expr type: {}", expr)
    sys.exit(1)


def GenerateErrorFormFromExpr(expr, error_type, upper_bound, M2,
                              eq_gids = [], constraint_exprs = []):
    logger.dlog("Calling FPTaylor for the first derivatives.")
    ...
    logger.dlog("First derivatives acquired")
    ...
    for tt in text_terms:
        ...
        if (error_type == "abs"):
            ...
            str_error = "({} /$-1 {})".format(str_error, str_program)
        else:
            logger.error("Invalid error_type: {}", error_type)
            sys.exit(1)
...
def CheckM2WithFPTaylor(fname_config):
    ...
    command_fpt = None
    if fname_config == "":
        command_fpt = "{} {}".format(os.environ["FPTAYLOR"], FPTAYLOR_M2_FQUERY)
    else:
        command_fpt = "{} -c {} {}".format(os.environ["FPTAYLOR"],
                                           fname_config,
                                           FPTAYLOR_M2_FQUERY)
    fpt_verify = subp.Popen(command_fpt, shell=True,
    ...
    logger.dlog("Check M2({}), M2: {} Total: {}",
                fname_config, err_M2, err_total)
...
def EnsureM2(alloc):
    ...
    for texpr in TARGET_EXPRS:
        ...
        if os.path.isfile(FPTAYLOR_M2_FQUERY):
            logger.warning("Overwriting file: {}", FPTAYLOR_M2_FQUERY)

        tft_ir_backend.ExportExpr4FPTaylorSanitation(texpr,
                                                     alloc,
                                                     FPTAYLOR_M2_FQUERY)

        cfg_verify = "{}/{}".format(os.environ["FPTAYLOR_BASE"],
                                    tft_utils.FPT_CFG_VERIFY)
        ...
        if err_total is None or err_total > error_threshold:
            cfg_verify_detail = "{}/{}".format(os.environ["FPTAYLOR_BASE"],
                                               tft_utils.FPT_CFG_VERIFY_DETAIL_BB)
        ...
        if err_total > error_threshold:
            logger.dlog("Total error is higher than the threshold by {}",
                        float(err_total - error_threshold))
...
def SolveErrorForms(eforms = [], optimizers = {}):
    ...
    while True:
        ...
        if alloc is None or tft_utils.NO_M2_CHECK:
            break

        # -- check the effect of M2 --
        time_m2 = time.time()
        err_exc = EnsureM2(alloc)
        tft_utils.TIME_CHECK_M2 += time.time() - time_m2

        if type(err_exc) is bool:
            if err_exc:
                break
            logger.error("Invalid return value of boolean EnsureM2")
            sys.exit(1)

        if type(err_exc) is float:
            assert(err_exc > 0.0)

            alloc  = None
            err_M2 = err_M2 + (err_exc / 2.0)

            logger.dlog("Actual error higher than threshold, retune with M2 = {}",
                        err_M2)
        ...
        logger.error("Invalid return value type of EnsureM2: {}", err_exc)
        sys.exit(1)
...
def SolveExprs(fname_exprs, optimizers = {}):
    ...
    logger.dlog("Parsing input expression.")
    ...
    try:
        with open(fname_exprs, "r") as f:
            ilines = f.readlines()
    except:
        logger.error("Cannot read file: {}", fname_exprs)
        sys.exit(1)

    ilines = [l.strip() for l in ilines]
    ilines = [l for l in ilines if l != ""]
    ilines = [l for l in ilines if not l.startswith("#")]
    ...
    while True:
        ...
        if tokens[0] != "opt-error-form":
            logger.error("Unknown option setting: {}", ilines[0])
            sys.exit(1)
    ...
    while True :
        ...
        for i in range(len(str_eps_list)):
            try:
    ...
    while True :
        ...
        if (gp12 not in EQ_GIDS
            and gp21 not in EQ_GIDS):
            EQ_GIDS.append(gp12)
    ...
    for te in TARGET_EXPRS:
        logger.dlog("Generating error forms for: {}", te)
